...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    [self.window setTitle:[NSString stringWithFormat:@"广财EasyDrcomGUI For OSX, v%@", version]];
    [self.lblBuild setStringValue: [NSString stringWithFormat:@"Code by HowquaX, build on %s %s.", __DATE__, __TIME__]];
    
    SYS_LOG_INFO("EasyDrcomGUI For OSX, v" << [version UTF8String] << std::endl);
    SYS_LOG_INFO("Code by HowquaX, build on " __DATE__ " " __TIME__ << std::endl);
    ...
    /*curl = curl_easy_init();
    curl_easy_setopt(curl, CURLOPT_URL, "http://58.62.247.115");
...
- (IBAction) connectClicked:(id)sender
{
    ...
    if ([btnValue isEqualToString:@"连接"])
    {
        ...
        //set the connectMode to WorkDistrict, due to it is only workable Mode in GDUFE.
...
- (IBAction) supportClicked:(id)sender {
    NSWorkspace *workspace = [NSWorkspace sharedWorkspace];
    [workspace openURL:[NSURL URLWithString:@"https://github.com/HowquaX/EasyDrcomGUI-For-GDUFE"]];
}
